<template>
  <div v-editable="blok">
    ...
    <div class="container mx-auto mb-12">
      ...
      <h1 :if="blok.data" class="text-6xl text-[#000000] font-bold mt-12 mb-4">{{ new Date(raw.published_at).toLocaleDateString('en-gb', { month: 'long', day: 'numeric', year: 'numeric' }).replace(/(\s+\d{4})$/, ',$1') }}</h1>
      <h2 class="text-2xl text-[#1d243d] font-bold mb-4">
        {{ blok.teaser }}
      </h2>
      <div class="flex space-x-2" v-if="blok.categories">
        <h2 v-for="category in blok.categories" class="text-2xl text-[#ff6e07] font-bold mb-4 ">
          {{ category.name }}
        </h2>
      </div>

      <article v-html="resolvedRichText" class="prose prose-xl max-w-none prose-ol:text-black "></article>
...
const { data } = await storyblokApi.get('cdn/stories', {
  ...
  filter_query: {
    ...
    categories: {
      any_in_array: props.raw.content.categories[0].uuid
    }
  }
})
